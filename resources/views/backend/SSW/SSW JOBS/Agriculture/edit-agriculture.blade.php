...
@section('title', 'Edit Agriculture')
@section('content')
    ...
    <form class="row g-3" method="post" action="{{(route('edit-agriculture'))}}"  enctype="multipart/form-data">
        @csrf
        <input type="hidden" id="food_id" name="agriculture_id" value="{{ $agriculture->id }}">
        <div class="col-md-6">
            <label for="inputName5" class="form-label">Agriculture Title</label>
            <input type="text" class="form-control" id="inputName5" value="{{$agriculture->agriculture_title}}" name="agriculture_title" placeholder="Agriculture Title" required>
        </div>
        <div class="col-md-6">
